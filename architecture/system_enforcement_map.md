...
| `guild-applications/{appId}` | Admin | Never (server only) | Guild applications |
| `guild-directory/{memberId}` | All auth | Never (server only) | Guild member listings |
| `guild-pairings/{pairingId}` | Guild member, Student, Admin | Never (server only) | Guild member-student pairs |
...
|---|---|---|---|
| `guild-docs/{userId}/*` | 10MB | PDF, JPEG, PNG, WebP | Owner, Admin |
...
|---|---|---|
| Auth token required for all guild/consulting actions | `api/guild-member.js`, `api/guild.js`, `api/consulting.js` | 401 returned |
| Admin email check for approve/reject | `api/guild-member.js:109` | 403 returned |
| Handle format: `/^[a-zA-Z0-9_-]{3,20}$/` | `api/user-actions.js:16` | 400 returned |
| Bio max length: 500 chars | `api/guild-member.js:409` | 400 returned |
| Guild member capacity: 1-20 | `api/guild-member.js:428` | 400 returned |
| Guild post title max: 200 chars | `api/guild-member.js:22` | Validated |
| Guild post body max: 10,000 chars | `api/guild-member.js:23` | Validated |
| Guild max images: 4 | `api/guild-member.js:24` | Validated |
| Syllabus text min: 50 chars | `api/guild-member.js` (teacher handler) | 400 returned |
| TTS max chars: 290 | `api/tts.js` | Enforced |
| Chat rate limit: 10 req/min | `api/chat.js:36-37` | 429 returned |
| Consulting types: character, narrative, coaching, media, adventure | `api/guild-member.js:19` | Set validation |
| Valid action routing in guild-member.js | `api/guild-member.js:89` | 400 for unknown actions |
...
| Guild doc max: 10MB | `src/profile/guildUpload.js` | Client-side reject |
...
|---|---|---|
| Guild eligibility: credential level >= 2 | `src/profile/guildEngine.js:83` | Only qualified users can apply |
| Required guild courses: monomyth-explorer, celestial-clocks-explorer, atlas-conversationalist | `api/guild-member.js:24`, `api/guild.js:32-36` | Guild activation + forum posting |
| Consulting engagement stages follow client-type template | `api/guild-member.js:27-67` | Stage structure integrity |
...
| Pairing capacity check: guild member at capacity → reject | `api/guild-member.js:612` | Prevents over-assignment |
| Duplicate pairing check | `api/guild-member.js:561-568` | One active pairing per pair |
...
### `src/tests/dataIntegrity.test.js` (306 tests)
...
| Pantheon files (237 tests) | 79 files discovered, valid JSON, 3+ deities, name fields (79 files × 3 checks) |
...
| Journey pattern integrity (5 tests) | 6 journeys have 8 stops, planetary has 7, zodiac has 12, cosmic uses YBR (26), 10 total definitions |
| Wheel traditions integrity (14 tests) | 9 traditions with expected IDs, required fields, quadrantColors N/E/S/W rgba format, direction structure, all 12 association keys per direction, cardinal angle convention (N=-90 E=0 S=90 W=180), null angles for Above/Below, color objects (name/hex/rgba), 6-dir traditions have Above+Below, descriptions are 2+ sentences |
...
| Guild member action routing | Valid action dispatch, unknown action rejection |
...
**Total: 899 tests across 13 suites + 13 runtime schemas**
